...
from lib.core.utils import non_max_suppression, letterbox, split_for_trace_model, scale_coords, driving_area_mask, lane_line_mask
...
def run_inference(
    ...
    input_image, _, (pad_w, pad_h) = letterbox(input_image)
    ...
    # 後処理
    # 車検出
    result_dets = split_for_trace_model(
        result_dets,
        anchor_grid,
    )

    result_dets = non_max_suppression(
        result_dets,
        conf_thres=score_th,
        iou_thres=nms_th,
    )
    ...
    # 路面セグメンテーション
    result_road_seg = driving_area_mask(
        results[4],
        (pad_w, pad_h),
    )

    # レーンセグメンテーション
    result_lane_seg = lane_line_mask(
        results[5],
        (pad_w, pad_h),
    )
